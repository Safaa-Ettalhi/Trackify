...
const TrailerList = ({ onEdit, onCreate, refreshTrigger }) => {
  ...
  const getStatusColor = (etat) => {
    switch(etat) {
      ...
      case 'utilisee': return 'bg-blue-100 text-blue-700';
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Liste des Remorques</h2>
        <button
          onClick={onCreate}
        ...
        >
          ...
          <span>Ajouter une remorque</span>
        </button>
      </div>
      ...
      {trailers.length > 0 ? (
                  ...
                  <tr key={trailer._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm font-medium text-gray-900">#{trailer.numero}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{trailer.type}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {trailer.capacite ? `${trailer.capacite} tonnes` : '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(trailer.etat)}`}>
                        {getStatusLabel(trailer.etat)}
                      </span>
                    </td>
        ...
        <div className="bg-white rounded-2xl p-12 border border-gray-200/50 shadow-lg text-center">
          ...
          <h3 className="text-xl font-bold text-gray-900 mb-2">Aucune remorque enregistrée</h3>
          <p className="text-gray-500 mb-6">Commencez par ajouter votre première remorque</p>
          <button
            onClick={onCreate}
